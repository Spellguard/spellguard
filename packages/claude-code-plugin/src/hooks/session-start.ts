...
import { ensureStableHelper, writeGitConfigEnv } from '../lib/env-file-writer';
...
import { healSessionEnvIdentity } from '../lib/session-env-identity-heal';
import {
...
// descriptor.
//
// C2: the status() REST probe can additionally return 'superseded' (a
// rotated-out token that still has an active replacement on the server). That
// value never appears on a wire push frame, so it is intentionally NOT part of
// the shared push-frame descriptor — widen it locally for the probe consumers
// (session-start + credential-monitor) instead of touching the shared protocol.
...
export async function runSessionStart(
  ...
  // so an agent that never reached the GitHub-ready path (identity-only, or a
  // revoked credential) stayed stuck at the server's creation-time framework.
  // Fire-and-forget; never blocks startup; depends only on identity.
  if (
  ...
  // is threaded into the env-file writes so the rules out-specify a force-SSH
  // rule for this exact repo.
  ...
  // Fail-closed: from here down we're committed to an HTTPS-remote
  // path where the host's credential helpers (e.g. a `gh auth setup-git`-
  // installed PAT) could otherwise serve github.com auth. Validate
  // CLAUDE_ENV_FILE and inject the credential-helper override now so every
  // failure path below (revoked, status-check failed, expired) keeps
  // git's auth flow pinned to the spellguard helper. The
  // spellguard-git-helper itself returns no credentials when config is
  // missing/revoked or the cached token is gone, so git push fails clean
  // rather than silently leaking the host PAT.
  const envFilePath = deps.claudeEnvFile ?? process.env.CLAUDE_ENV_FILE;
  if (!envFilePath) {
    renderMessage({
      level: 'error',
      message:
        'Spellguard: CLAUDE_ENV_FILE environment variable not set; cannot inject git credential helper.',
    });
    return { ok: false, reason: 'no_claude_env_file' };
  }
  // Per-agent gh session dir — exported as GH_CONFIG_DIR so the session's gh CLI
  // resolves the scoped token (the daemon maintains its hosts.yml). Never the
  // developer's real ~/.config/gh.
  ...
  if (error) {
    ...
    // agent record no longer exists on the server. Surface an actionable
    // reconnect prompt instead of a bare status code. Other failures
    // (network, 5xx) are treated as transient.
  ...
  // C2: 'superseded' = this scoped token was ROTATED OUT, but an active
  // replacement exists on the server. Do NOT whole-config-revoke — that would
  // brick the agent over a routine rotation (the bug that took down a live
  // agent). Fall through to spawn the daemon: its Resume divergence re-delivers
  // the new credential via admin_reissue → credential_request → the fresh token
  // lands on disk. The old token stays usable for the brief rotation window
  // (GitHub installation tokens aren't revoked by our DB rotate; they expire
  // naturally ~1h), so git keeps working in the meantime.
  if (status === 'superseded') {
    renderMessage({
      level: 'info',
      message:
        'Spellguard: your GitHub credential was rotated; fetching the new token over the credential channel.',
    });
    // intentional fall-through to the daemon-spawn path below
  }
  ...
  if (deps.writeEnvFileImpl) {
    ...
    // Resume-staleness fix: a resumed session re-runs SessionStart and writes a
    // fresh capture above, but Claude Code keeps the ORIGINAL startup capture
    // too and sources both (last-wins by filename), so a stale
    // `(Spellguard:<old-agent>)` identity from before a re-provision would
    // override this fresh write. Normalize the identity across all sibling
    // captures so the current value wins. Best-effort (see the helper).
  ...
  // The commit filesystem watcher runs inside the credential daemon
