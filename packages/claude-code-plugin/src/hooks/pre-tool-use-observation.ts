 ...
 * HTTP 401/403/404/410 we return a block decision to Claude Code. Under the
 ...
 *
 * Git-op detection uses the `detectGitOp` tokenized parser
 * (see `../lib/git-command-parser`) rather than naive substring matching.
 */
import {
...
async function probeStatus(args: {
  ...
  try {
    ...
    // /credentials/github/status returns
