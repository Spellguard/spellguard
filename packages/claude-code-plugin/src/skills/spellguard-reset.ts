...
import { clearGhTokenFile } from '../lib/gh-token-file';
...
export function wipeSupersededCredentials(args: SupersededWipeArgs): void {
  ...
  // 2. Clear the env-file git-helper exports (Claude Code reads CLAUDE_ENV_FILE)
  //    so the next git op fails closed rather than presenting a stale token.
  if (args.envFilePath) {
    try {
      (args.clearGitConfigEnvImpl ?? clearGitConfigEnv)(args.envFilePath);
    } catch {
      /* best-effort — env file may already be gone */
    }
  }
  ...
  // 4. Drop the GH_TOKEN file so a `claude` startup auto-update can't keep
  //    exporting the wiped token (mirrors the gh-session clear above).
  if (args.ghTokenFile) {
    try {
