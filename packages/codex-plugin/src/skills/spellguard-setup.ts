 ...
 * acquire those server-side, so it polls the new endpoint every 2 s up to
 * 10 minutes.
 *
 ...
 * just this plugin's canonical wire/DB slug ({@link FRAMEWORK}) — the SAME value
 * sent on the bootstrap_request frame. Recorded server-side on the nonce via
 * POST /v1/bootstrap/register-framework and used to scope the dashboard's
 * "select existing agent" list (FR-7) to same-framework agents owned by the
 * authenticated user. Plugin-asserted, NOT user-typed.
 */
...
export async function promptExistingConfigChoice(opts: {
  ...
  // input, CI, non-interactive shells). Fail fast with the safest default —
  // 'print_identity' does not mutate server state.
  if (!opts.promptFn && !process.stdin.isTTY) {
    renderMessage({
      level: 'warn',
      message:
        'Spellguard: an existing credential is present and no interactive terminal is available. Printing the current identity. To act on it non-interactively, re-run with --choice reauthorize | additional | print.',
    });
...
export async function runSpellguardSetup(
  ...
  if (existing.config && !existing.config.revoked && !probeSaysGone) {
    ...
    if (choice === 'provision_additional') {
      ...
      // overwritten at the end of the flow — MVP limitation; multi-local-
      // credential storage is not in scope for this PRD.
      renderMessage({
        level: 'info',
        message:
  ...
  // Replace awaitBootstrap with AgentControlClient in nonce mode.
  ...
  // (agent-control/credential-handlers.ts) populates `scopedToken` /
  // `gitAuthorName` / `gitAuthorEmail` / `scopeSummary` when the GitHub
  // credential is later delivered (via the dashboard's GitHub-App install
  // OAuth callback at `POST /v1/integrations/github/complete-install`,
  // which calls `pushToAgentChannel` to broadcast the descriptor).
  //
  ...
  // fields inline so the daemon can use them immediately, matching the
  // earlier behavior. Operators on a current server walk through the
  // dashboard "Connect GitHub" step after bootstrap.
  ...
  // session-start hook for this. On a fresh mid-session install nothing
  // else can start a daemon before the user's next session, and the GitHub
  // credential the dashboard pushes right after this flow would sit queued
  // server-side with no consumer (2026-06-11 incident).
  ...
  const daemonLine =
    daemonResult.daemon === 'spawned'
      ...
      : daemonResult.daemon === 'already-running'
        ...
        : `  WARNING: the credential daemon could not be started (${daemonResult.reason}); restart your Codex session so the session-start hook can start it.`;
  renderMessage({
    level: 'info',
    message: [
 ...
 * the server to push `credential_delivered{cause:'bootstrap'}`. Replaces
 * the legacy `awaitBootstrap` (websocket-bootstrap.ts).
 *
 * The client sends `bootstrap_request{agent_name, statement_of_reason}`
  ...
  return new Promise<BootstrapClientResult>((resolve) => {
    ...
    client = new AgentControlClient({
      ...
      onFatalClose: (code, reason) => {
        ...
        // The previous wrapper said "timed out or channel unavailable" for
        // every code, hiding the typed bootstrap-error reasons
