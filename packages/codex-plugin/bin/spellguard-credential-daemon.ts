...
// ── REQ-C03: Codex login-relay gate (exported for unit tests) ────────────────

/**
 * Compute whether the Codex device-auth login relay should AUTO-START at boot.
 * Injectable deps so tests can control all inputs without touching the real
 * filesystem, process.env, or the `codex` CLI.
 *
 ...
 *        - `'wait'` → enabled but USER-INITIATED: the daemon wires the relay
 *                     (the `onLoginRestart` handler is always registered) but
 *                     does NOT spawn `codex login --device-auth` at boot. The
 *                     relay starts only when the operator clicks "Start login"
 *                     in the dashboard, which sends a `login_restart` down-frame.
 *      The codex box ships with `'wait'` so the one-time device code is minted
 *      WHILE the operator is present (it expires in ~15 min) instead of during
 *      the minutes-long provision while they are away.
 *   3. The box is NOT already logged in to Codex. Unlike Claude, Codex has no
 *      OAuth-token env var: success is the credential Codex writes to
 *      `~/.codex/auth.json`, detected via `codex login status`. The daemon runs
 *      that status check once at startup and passes the boolean here.
 *   4. The on-disk login-done sentinel (`~/.codex-login-done`) does NOT exist
 *      (the relay writes it on success — authoritative for daemon restarts,
 *      and cheaper than re-shelling `codex login status` every boot once login
 *      has already completed).
 *
 * Either signal of "already authorized" (the live status check OR the sentinel
 * file) suppresses the relay — we never re-login a box that is already logged
 * in. `autoStart` ONLY gates the boot-time auto-spawn; the operator-initiated
...
export function computeNeedsLoginRelay(opts: {
  ...
  loginDoneFileExists: boolean;
}): boolean {
  return (
    opts.relayEnabled &&
    opts.autoStart &&
    !opts.alreadyLoggedIn &&
    !opts.loginDoneFileExists
  );
}

/**
 * REQ-C03 — Start (or restart) the in-box Codex device-auth login relay and
 * make a STARTUP failure observable to the dashboard.
 *
 * Why this exists (mirrors Claude's TC-005): relay startup has two awaited
 ...
 *   2. the `codex login --device-auth` pty spawn inside `runLoginRelay`, which
 *      can throw synchronously if the binary is missing.
 ...
 * the dashboard's Authorize-Codex card hangs forever on "waiting…".
 *
 ...
 * `authorized` BEFORE logging — so the DO persists `login_state='failed'` and
 * the dashboard shows the generic failed state + Restart action. Graceful-
 * degrade is preserved: the credential socket keeps working; only the relay is
 * degraded. Aborts (login_restart killed the old relay) are NOT surfaced as
 * failures — the new relay owns the dashboard state.
 *
...
async function main() {
  ...
  // session's config.toml block — not just the host-level rules 1/2. The daemon
  // is single-cwd-per-agent (see header), so its `cwd` is the repo it serves.
  ...
  // ── REQ-C03: Codex device-auth login-relay gate ────────────────────────────
  //
  // Start the relay ONLY when the managed boot path enabled it AND the box is
  // not already authenticated. Two independent "already authorized" signals
  // suppress it so a daemon restart never re-logs-in:
  //
  //   1. `~/.codex-login-done` sentinel file — written by the relay handler
  //      (defaultMarkAuthenticated) on success. Authoritative + cheap on restart.
  //   2. A live `codex login status` check (defaultIsLoggedIn). Catches the case
  //      where Codex is logged in (auth.json present) but the sentinel was never
  //      written (e.g. login completed out-of-band, or the box image shipped
  //      already-authed).
  //
  // Gated additionally on SPELLGUARD_LOGIN_RELAY (only the managed systemd unit
  // sets it) so the relay never runs on a developer machine or ordinary restart.
  // The VALUE selects the start mode (REQ-C03 user-initiated change):
  //   - '1'    → relay enabled + AUTO-START at boot (legacy behaviour).
  //   - 'wait' → relay enabled but USER-INITIATED: wire the relay handlers but
  //              do NOT spawn `codex login --device-auth` at boot; wait for the
  //              operator's "Start login" (a `login_restart` down-frame). The
  //              codex box ships 'wait' so the ~15-min device code is minted
  //              while the operator is present, not during the away-from-keyboard
  //              provision. The `onLoginRestart` handler is wired regardless.
  ...
  const loginDoneFileExists = existsSync(LOGIN_MARKER_PATH);
  // Avoid shelling `codex login status` when the relay won't auto-start or the
  // sentinel already proves login — only run the (slower) live check when it can
  // actually change the gate's outcome (i.e. only on the auto-start path).
  const alreadyLoggedIn =
    loginRelayEnabled && loginRelayAutoStart && !loginDoneFileExists
      ? await defaultIsLoggedIn()
      : false;
  ...
  // AbortController for the currently in-flight login relay. Replaced on each
  // login_restart so the old relay's pty is killed before the new one starts —
  // prevents two concurrent `codex login --device-auth` ptys running at once.
  ...
  const client = new AgentControlClient({
    ...
    // REQ-C03 (FIND-DA28 analog) — on every (re)connect, RE-ASSERT the terminal
    // login state when the box is already authenticated. The relay's one-shot
    // login_relay_update{authorized} is fire-and-forget; if the channel was
    // mid-churn at login completion the update is silently dropped and — because
    // `authorized` is terminal — nothing ever re-sends it, so the dashboard
    // sticks on "Authorize Codex" even though the box is authenticated. The
    // login-done sentinel on disk == login succeeded, so re-send `authorized`
    ...
    // re-run, per the needsLoginRelay gate). No sentinel → no-op (a real login
    // still pending must not be reported authorized).
    onConnect: () => {
      // No sentinel == a real login is still pending; must NOT report authorized.
      if (!existsSync(LOGIN_MARKER_PATH)) return;
      // Settle before sending: onConnect fires on the raw socket 'open', which
      // can arrive BEFORE the DO has finished storing this connection's channel
      // identity. A login_relay_update sent in that window is rejected by the DO
      // (Ack{agent_unknown}) and never persists. A short delay lets the identity
      // settle; re-check the sentinel is still present before sending.
      setTimeout(() => {
        if (existsSync(LOGIN_MARKER_PATH)) {
    ...
    // REQ-C03: re-run the Codex device-auth relay on explicit operator request.
    //
    // This handler is ALWAYS registered — NOT gated on `needsLoginRelay`. Only
    // the INITIAL auto-start of the relay (below) is gated; handler registration
    // is not, so a "Restart login" click works even when the box was already
    // logged in (the operator wants a fresh login). A fresh device code is
    // minted (the 15-min code can expire). NOTE (vs Claude): Codex consumes
    // NOTHING back down the channel — there is deliberately NO `onLoginCode`
    // handler (the box polls OpenAI itself).
    onLoginRestart: () => {
      log('login_restart received — re-running Codex device-auth login relay');
      // Abort the currently in-flight relay (if any) so its pty is killed before
      // we start a new one. Without this, two concurrent `codex login
      // --device-auth` ptys run until the old one times out.
      ...
      // Route the restart through startLoginRelay so a startup failure (node-pty
      // load error / `codex login` can't spawn) is reported UP the channel as
      // login_relay_update{state:'failed'} — not just logged locally.
  ...
  // REQ-C03: start the Codex device-auth login relay when gated. The relay
  // drives `codex login --device-auth` in a pty, streams the device URL + the
  // one-time code UP the channel, then waits for the codex process to exit and
  // confirms with `codex login status` (the box polls OpenAI itself — nothing
  // comes back down). Non-fatal: if the relay fails, the daemon continues
  // serving the credential socket normally.
  if (needsLoginRelay) {
    log(
      'starting Codex device-auth login relay (SPELLGUARD_LOGIN_RELAY=1, not logged in, no sentinel)',
    );
    // Track the initial relay's controller so a login_restart can abort it.
    relayAbortController = new AbortController();
    const initialSignal = relayAbortController.signal;
    // startLoginRelay wraps the spawner+relay startup in try/catch and, on
    // failure, sends login_relay_update{state:'failed'} UP the channel (same
    ...
    // 'wait' mode (REQ-C03 user-initiated): the relay is wired but does NOT
    // auto-start. It boots "login-ready" and waits for the operator's "Start
    // login" click (a login_restart down-frame → onLoginRestart). The dashboard
    // card shows a Start button while login_state is still null.
    log(
    ...
    log('login relay skipped — Codex already logged in (sentinel or status)');
  }
